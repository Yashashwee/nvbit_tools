...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    // kernellist_filepath = "/home/yashashwee/gputejas/nvbit-Linux-x86_64-1.5.5/nvbit_release/traces/kernelslist";
    // is_per_core = true;
    return 0;
  } else if (argc == 2) {
...
void group_per_block(const char *filepath) {
  ...
  while (!ifs.eof()) {
    ...
    if (line.length() == 0 || line[0] == '#') {
      ...
      cout<<tb_id<<" "<<insts.size()<<" "<<found_block_dim<<" "<<found_grid_dim<<"\n";  
